function formatPace(movingTimeSec, distanceMeters) {
  ...
  function getQuickChartUrl(firstRun, latestRun, type = 'pace') {
    ...
    const data = type === 'pace'
      ? [parseFloat(firstRun.pace.replace(':', '.')) || 0, parseFloat(latestRun.pace.replace(':', '.')) || 0]
      : [parseFloat(firstRun.distance) || 0, parseFloat(latestRun.distance) || 0];
        ...
        datasets: [
          {
            ...
            fill: false,
            tension: 0.3,
            borderColor: type === 'pace' ? 'rgba(75,192,192,1)' : 'rgba(255,99,132,1)',
...
function generateRunningJourneySection(allRuns) {
  ...
  const longestRun = allRuns.reduce((a, b) => (a.distance > b.distance ? a : b));
...
module.exports = { formatPace, prepareRun, getQuickChartUrl, getConfidenceLevelText, generateRunningJourneySection, generateProgressTable };
